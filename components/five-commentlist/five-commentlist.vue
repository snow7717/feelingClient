	...
	<view class="view-content-comment" v-if="commentList.length > 0">
		...
		<view class="view-pro-comment-list" v-for="(item, index) in commentList" :key="index">
			...
			<view class="view-pro-comment-list-item">
				<image :src="item.commentator.avatar" class="view-comment-list-item-photo" mode="aspectFill"></image>

				<view class="comment-pro-title" style="flex: 1;">
					<text class="comment-name">{{ item.commentator.name }}</text>
				...
				<view class="comment-pro-title" style="align-items: center;">
					<image
						v-if="item.praises.indexOf(user._id) > -1"
						class="praise-icon"
					...
					<image
						...
						mode="aspectFill"
					></image>
					<text class="praise-num">{{ item.praises.length }}</text>
				</view>
			</view>
			<text class="pro-comment-content">{{ item.content }}</text>
			<view class="pro-comment-sub">
				<text class="pro-comment-sub-time">{{ item.created_at }}</text>
				<text @click="clickDelete(item)" class="pro-comment-sub-time" style="padding-left: 20rpx;" v-if='item.commentator._id == user._id'>删除</text>
			</view>
		</view>
	</view>
</template>
